...
def create_order_detail(request: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return order_details.create(db, request)
...
@app.get("/order_details/{order_details_id}", tags=["Order Details"])
def read_order_detail(order_details_id: int, db: Session = Depends(get_db)):
    return order_details.read_one(db, order_details_id)


@app.put("/order_details/{order_details_id}", tags=["Order Details"])
def update_order_detail(order_details_id: int, request: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    return order_details.update(db, order_details_id, request)
...
def delete_order_detail(order_details_id: int, db: Session = Depends(get_db)):
    order_details.delete(db, order_details_id)